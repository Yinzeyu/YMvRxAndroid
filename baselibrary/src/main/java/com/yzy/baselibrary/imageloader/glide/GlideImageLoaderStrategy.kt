        ...
        config.context?.let { context ->
            val glideRequest = GlideApp.with(context)
                ...
                .apply {
                    ...
                    //圆角
                    if (config.corner) {
                        transform(RoundedCornersTransformation(config.cornerRadius, config.margin, config.cornerType))
                    }
                    //自定义size
                    config.size?.let {
            ...
            if (config.url?.isNotBlank() == true && config.url?.toLowerCase()?.startsWith("http") == true && config.progress != null) {
                //只有网络图有加载进度的回调
                config.url?.let {
                    GlideHttpClientManager.addListener(url = it, listener = object : OnImageProgressListener {
                        override fun onProgress(
                            ...
                            config.progress?.invoke(url, isComplete, percentage, bytesRead, totalBytes)
                        }
                    })
                }
            }
