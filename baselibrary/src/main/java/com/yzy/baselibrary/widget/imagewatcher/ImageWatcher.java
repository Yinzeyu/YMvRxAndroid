...
class ImageWatcher extends FrameLayout
        implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    private final TypeEvaluator<Integer> mColorEvaluator = new TypeEvaluator<Integer>() {
        ...
        public Integer evaluate(float fraction, Integer startValue, Integer endValue) {
            ...
            int alpha =
                    (int) (Color.alpha(startColor) + f * (Color.alpha(endColor) - Color.alpha(startColor)));
            int red = (int) (Color.red(startColor) + f * (Color.red(endColor) - Color.red(startColor)));
            int green =
                    (int) (Color.green(startColor) + f * (Color.green(endColor) - Color.green(startColor)));
            int blue =
                    (int) (Color.blue(startColor) + f * (Color.blue(endColor) - Color.blue(startColor)));
            return Color.argb(alpha, red, green, blue);
        }
    };
    protected float scaleSensitivity = 0.3f;// 缩放灵敏度 越小越灵敏
    protected float edgeResilience = 0.16f; // 边缘阻尼弹性 越小越难拉动
    protected int mErrorImageRes;// 图片加载失败站位图
    ...
    private boolean isFirstJoin = true;//是否第一次进入
    private boolean isImageLoadFinish = false;//图片是否加载完成
    ...
    public ImageWatcher(Context context, AttributeSet attrs) {
        ...
        vPager.setOverScrollMode(ViewPager.OVER_SCROLL_NEVER);
    ...
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, final List<Uri> urlList) {
        ...
        if (i == null || imageGroupList == null || urlList == null || imageGroupList.size() < 1 ||
            ...
            String info = "i[" + i + "]";
            info += "#imageGroupList " + (imageGroupList == null ? "null"
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            if (Math.abs(moveX) > mTouchSlop || Math.abs(moveY) > mTouchSlop) {
                ...
                } else if (vsCurrent != null) {
                    if (Math.abs(moveX) < mTouchSlop && moveY > Math.abs(moveX) * 3 &&
                            vsDefault.height * vsCurrent.scaleY / 2 - vsDefault.height / 2
                                    <= iSource.getTranslationY()) {
                        ...
                        if ("horizontal".equals(imageOrientation)) {
                            ...
                            if (vsDefault.width * vsCurrent.scaleX <= mWidth) {
                                if (Math.abs(moveY) < mTouchSlop
                                        && Math.abs(moveX) > mTouchSlop
                                        && Math.abs(moveX) > Math.abs(moveY) * 2) {
                                    mTouchMode = TOUCH_MODE_SLIDE;
                                }
                            } else {
                                float translateXRightEdge =
                                        vsDefault.width * vsCurrent.scaleX / 2f - vsDefault.width / 2f;
                                float translateXLeftEdge =
                                        mWidth - vsDefault.width * vsCurrent.scaleX / 2f - vsDefault.width / 2f;
    ...
    public boolean onSingleTapConfirmed() {
        ...
        if (vsDefault == null || (vsCurrent.scaleY <= vsDefault.scaleY
                && vsCurrent.scaleX <= vsDefault.scaleX)) {
            mExitRef = 0;
        } else {
    ...
    public void onLongPress(MotionEvent e) {
        if (pictureLongPressListener != null) {
            pictureLongPressListener.onPictureLongPress(iSource, mUrlList.get(vPager.getCurrentItem()),
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (mTouchMode != TOUCH_MODE_AUTO_FLING && mPagerPositionOffsetPixels == 0) {
            if (e1 != null && e2 != null && (Math.abs(e1.getX() - e2.getX()) > 50
                ...
                final ViewState vsCurrent = ViewState.write(iSource, ViewState.STATE_CURRENT);
                final ViewState vsDefault = ViewState.read(iSource, ViewState.STATE_DEFAULT);
                if (vsCurrent == null || vsDefault == null) {
                    return false;
                }
                ...
                float translateXEdge = vsDefault.width * (vsCurrent.scaleX - 1) / 2f;
                ...
                if (vsCurrent.scaleY * iSource.getHeight() > mHeight) {
                    ...
                    float translateYTopEdge =
                            vsDefault.height * vsCurrent.scaleY / 2f - vsDefault.height / 2f;
                    float translateYBottomEdge =
                            mHeight - vsDefault.height * vsCurrent.scaleY / 2f - vsDefault.height / 2f;
    ...
    private void handleDragGesture(MotionEvent e2, MotionEvent e1) {
        ...
        final String imageOrientation = (String) iSource.getTag(R.id.image_orientation);
        if ("horizontal".equals(imageOrientation)) {
            float translateXEdge = vsDefault.width * (vsTouchDrag.scaleX - 1) / 2f;
            ...
            if (vsDefault.width * vsTouchDrag.scaleX <= mWidth) {
                ...
                float translateXRightEdge =
                        vsDefault.width * vsTouchDrag.scaleX / 2f - vsDefault.width / 2f;
                float translateXLeftEdge =
                        mWidth - vsDefault.width * vsTouchDrag.scaleX / 2f - vsDefault.width / 2f;

                if (translateXValue > translateXRightEdge) {
                    translateXValue =
                            translateXRightEdge + (translateXValue - translateXRightEdge) * edgeResilience;
                } else if (translateXValue < translateXLeftEdge) {
                    translateXValue =
                            translateXLeftEdge + (translateXValue - translateXLeftEdge) * edgeResilience;
                }
            }
            iSource.setTranslationX(translateXValue);
        }
        ...
        if (vsDefault.height * vsTouchDrag.scaleY > mHeight) {
            float translateYTopEdge = vsDefault.height * vsTouchDrag.scaleY / 2f - vsDefault.height / 2f;
            float translateYBottomEdge =
                    mHeight - vsDefault.height * vsTouchDrag.scaleY / 2f - vsDefault.height / 2f;
            if (translateYValue > translateYTopEdge) {
                translateYValue =
                        translateYTopEdge + (translateYValue - translateYTopEdge) * edgeResilience;
            } else if (translateYValue < translateYBottomEdge) {
                translateYValue =
                        translateYBottomEdge + (translateYValue - translateYBottomEdge) * edgeResilience;
            }
    ...
    private void handleScaleGesture(MotionEvent e2) {
        ...
        float centerX = (e2.getX(1) + e2.getX(0)) / 2f;
        float centerY = (e2.getY(1) + e2.getY(0)) / 2f;
    ...
    private void handleScaleTouchResult() {
        ...
        if (iSource.getHeight() * vsCurrent.scaleY > mHeight) {
            ...
            float translateYBottomEdge = vsDefault.height * vsCurrent.scaleY / 2f - vsDefault.height / 2f;
            float translateYTopEdge =
                    mHeight - vsDefault.height * vsCurrent.scaleY / 2f - vsDefault.height / 2f;
    ...
    private void handleDragTouchResult() {
        ...
        if ("horizontal".equals(imageOrientation)) {
            ...
            if (vsDefault.width * vsCurrent.scaleX <= mWidth) {
                ...
                float translateXRightEdge = vsDefault.width * vsCurrent.scaleX / 2f - vsDefault.width / 2f;
                float translateXLeftEdge =
                        mWidth - vsDefault.width * vsCurrent.scaleX / 2f - vsDefault.width / 2f;
    ...
    public boolean handleBackPressed() {
        return isInTransformAnimation || (iSource != null
     ...
     */
    private void animSourceViewStateTransform(View view,
                                              final ViewState vsResult) {
        if (view == null) {
            return;
        }
        if (animImageTransform != null) {
        ...
        animBackground.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (stateChangedListener != null) {
                    stateChangedListener.onStateChangeUpdate(ImageWatcher.this, iSource, getCurrentPosition(),
                            getDisplayingUri(), p, tag);
                }
            }
        });
        animBackground.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                ...
                if (stateChangedListener != null) {
                    if (tag == STATE_ENTER_DISPLAYING) {
                        isFirstJoin = true;
                        stateChangedListener.onStateChanged(ImageWatcher.this, getCurrentPosition(),
                                getDisplayingUri(), tag);
                    }
                }
            }
            ...
            public void onAnimationEnd(Animator animation) {
                if (stateChangedListener != null) {
                    ...
                    if (tag == STATE_ENTER_DISPLAYING && !isImageLoadFinish) {
                        //图片未加载完成时，展示loading
                        adapter.notifyItemChangedState(initPosition, true, false);
                    }
                }
                isFirstJoin = false;
                if (tag == STATE_EXIT_HIDING) {
    ...
    private void animFling(View view, final ViewState vsResult, long duration) {
        if (duration > 800) {
            duration = 800;
        ...
        animFling =
                ViewState.restoreByAnim(view, vsResult.mTag).addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationStart(Animator animation) {
                        mTouchMode = TOUCH_MODE_AUTO_FLING;
                    }
    ...
    public interface LoadCallback {
        void onResourceReady(File imageFile);
    ...
    public interface OnStateChangedListener {
        void onStateChangeUpdate(ImageWatcher imageWatcher, View clicked, int position,
    ...
    public interface OnPictureLongPressListener {
         ...
         */
        void onPictureLongPress(View v, Uri uri, int pos); // 当前展示图片长按的回调
    }
    ...
    public class DefaultIndexProvider implements IndexProvider {
        ...
        public View initialView(Context context) {
            ...
            float tCurrentIdxTransY =
                    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30, displayMetrics) + 0.5f;
    ...
    public class DefaultLoadingUIProvider implements LoadingUIProvider {
        final LayoutParams lpCenterInParent =
                new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
    ...
    class ImagePagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(@NonNull ViewGroup container, int position) {
            ...
            LargeImageView imageView = new LargeImageView(container.getContext());
            ...
            if (loadingUIProvider != null) {
                loadView = loadingUIProvider.initialView(container);
            }
            if (loadView == null) {
        ...
        void notifyItemChangedState(int position, boolean loading, boolean error) {
            View imageView = mImageSparseArray.get(position);
            if (imageView != null) {
        ...
        private boolean setDefaultDisplayConfigs(final LargeImageView imageView,
                                                 final int pos,
                                                 boolean hasPlayBeginAnimation) {
                ...
                if (originRef != null) {
                    ...
                    if (bmpMirror != null) {
                        ...
                        ViewState vsThumb = ViewState.write(imageView, ViewState.STATE_THUMB)
                                ...
                                .translationX((mWidth - bmpMirrorWidth) / 2f)
                                .translationY((mHeight - bmpMirrorHeight) / 2f);
                        imageView.setImage(bmpMirror);
                    ...
                    ViewState.write(imageView, ViewState.STATE_ORIGIN).alpha(0).scaleXBy(0.5f).scaleYBy(0.5f);
                    ViewState vsThumb = ViewState.write(imageView, ViewState.STATE_THUMB)
                            .alpha(0);
                    animSourceViewStateTransform(imageView, vsThumb);
                }
            ...
            loader.load(imageView.getContext(), mUrlList.get(pos), new LoadCallback() {
                @Override
                public void onResourceReady(@Nullable File imgFile) {
                    Log.e("CASE", "imgFile=--------------");
                    isImageLoadFinish = true;
                    final int sourceDefaultWidth, sourceDefaultHeight, sourceDefaultTranslateX,
                            sourceDefaultTranslateY;
                    int size = imageView.getContext().getResources().getDisplayMetrics().widthPixels;
                    int resourceImageWidth = size;
                    int resourceImageHeight = size;
                    if (imgFile != null) {
                        Log.e("CASE", "imgFile=" + imgFile.getPath());
                        BitmapFactory.Options options = new BitmapFactory.Options();
                        options.inJustDecodeBounds = true;
                        BitmapFactory.decodeFile(imgFile.getPath(), options);
                        resourceImageWidth = options.outWidth;
                        resourceImageHeight = options.outHeight;
                    }
                    if (resourceImageWidth * 1f / resourceImageHeight > mWidth * 1f / mHeight) {
                        sourceDefaultWidth = mWidth;
                        sourceDefaultHeight =
                                (int) (sourceDefaultWidth * 1f / resourceImageWidth * resourceImageHeight);
                    ...
                    if (imgFile != null) {
                        imageView.setImage(new FileBitmapDecoderFactory(imgFile));
                    } else {
                        imageView.setImage(R.drawable.img_placeholder_ffd9d3eb);
                    }
                ...
                public void onLoadFailed(Drawable errorDrawable) {
                    notifyItemChangedState(pos, false, true);
                }
            });
