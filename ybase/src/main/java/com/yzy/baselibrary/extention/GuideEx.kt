...
//显示新手引导
fun Fragment.extShowGuide(
    rootView: FrameLayout = requireActivity().mContentView,//需要显示引导的根布局(fragment需要传)
    ...
    //获取引导View
    val guideView =
        view ?: LayoutInflater.from(requireContext()).inflate(layoutId, rootView, false) ?: return
            ...
            override fun onGlobalLayout() {
                ...
                if (guideView.height > 0) {
                    ...
                    if (isFullGuide) {
                        ...
                        //为了解决间距问题，需要外部嵌套一层
                        val guideViewRoot = FrameLayout(requireContext())
...
//隐藏新手引导:不传view表示全部关闭
private fun Fragment.extHideGuide(
    rootView: FrameLayout = requireActivity().mContentView,//需要显示引导的根布局(fragment需要传)
    view: View? = null
) {
    //移除传入的view
