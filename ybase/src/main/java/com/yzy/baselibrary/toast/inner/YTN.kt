...
import com.yzy.baselibrary.toast.inner.ActivityToast
 ...
 *@author: yzy.
 */
class YTN : Handler(Looper.getMainLooper()) {


    private val REMOVE = 2
    private var toastQueue: DPriorityQueue<BaseToast>//列表中成员要求非空

    init {
        toastQueue = DPriorityQueue(Comparator<BaseToast> { x, y ->
    ...
    private fun notifyNewToastComeIn(mToast: BaseToast) {
    ...
    private fun remove(toast: BaseToast) {
    ...
    private fun removeInternal(toast: BaseToast?) {
    ...
    private fun sendRemoveMsgDelay(toast: BaseToast) {
        removeMessages(REMOVE)
    ...
    private fun sendRemoveMsg(toast: BaseToast) {
        removeMessages(REMOVE)
        val message = obtainMessage(REMOVE)
        message.obj = toast
        sendMessage(message)
    ...
    private fun displayToast(toast: BaseToast) {
        ...
        if (parent is ViewManager) {
            (parent as ViewManager).removeView(toastView)
        }
        ...
        try {
            ...
            if (e is WindowManager.BadTokenException &&
                ...
                if (toast is ActivityToast) {
                    ...
                    BaseToast.Count4BadTokenException = 0
                } else {
                    BaseToast.Count4BadTokenException++
                    //尝试使用ActivityToast
